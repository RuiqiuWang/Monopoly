...
.PHONY: all test cli engine tutorial_test json_test clean

all: test_movement movement_cli tutorial_test game_engine
...
game_engine: game_engine.c tutorial.c map.c include/map.h include/tui.h include/tutorial.h include/movement.h include/player.h include/block_bit_utils.h
	$(CC) $(CPPFLAGS) $(CFLAGS) game_engine.c tutorial.c map.c tui.c movement.c -o $@
...
clean:
	rm -f test_movement test_movement.exe movement_cli movement_cli.exe tutorial_test tutorial_test.exe game_engine game_engine.exe tests/json_runner tests/json_runner.exe
